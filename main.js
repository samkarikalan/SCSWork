...
sheet.innerHTML = `<div class="mode-sheet-handle"></div> <div class="mode-sheet-title">Switch Mode</div> <div class="mode-sheet-options"> <button class="mode-sheet-btn viewer ${appMode === 'viewer' ? 'active-viewer' : ''}" onclick="switchMode('viewer')"> <div class="mode-sheet-icon">👁</div> <div class="mode-sheet-info"> <div class="mode-sheet-name">Viewer</div> <div class="mode-sheet-desc">Watch live rounds &amp; scores</div> </div> ${appMode === 'viewer' ? '<span class="mode-sheet-check">✅</span>' : ''} </button> <button class="mode-sheet-btn organiser ${appMode === 'organiser' ? 'active-organiser' : ''}" onclick="switchMode('organiser')"> <div class="mode-sheet-icon"><img src="win-cup.png" style="width:32px;height:32px;object-fit:contain;filter:drop-shadow(0 1px 4px rgba(0,0,0,0.25))"></div> <div class="mode-sheet-info"> <div class="mode-sheet-name">Round Organiser</div> <div class="mode-sheet-desc">Run session, score games, manage players</div> </div> ${appMode === 'organiser' ? '<span class="mode-sheet-check">✅</span>' : ''} </button> <button class="mode-sheet-btn vault ${appMode === 'vault' ? 'active-vault' : ''}" onclick="requestVaultMode()"> <div class="mode-sheet-icon" style="background:rgba(245,158,11,0.18)">🔑</div> <div class="mode-sheet-info"> <div class="mode-sheet-name">Vault Manager</div> <div class="mode-sheet-desc">Club admin — players, requests, management</div> </div> ${appMode === 'vault' ? '<span class="mode-sheet-check">✅</span>' : ''} </button> </div>`;
...
const overlay = document.getElementById(‘modeSheetOverlay’);
if (overlay) overlay.remove();
// Vault mode is freely accessible — individual vault pages
// (Modify, Club Mgmt) have their own admin password protection.
// Removing mode-level gate so first-time club creation is not blocked.
switchMode(‘vault’);
}
